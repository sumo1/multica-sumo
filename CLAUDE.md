...
## Task Mode (Goal Orchestration)

A top-level **任务 / Tasks** page (`packages/views/tasks/`, route `/{ws}/tasks`) where a PMO agent plans a goal into a subtask DAG and a dynamic squad executes it. Full design + rationale: `docs/task/260608-assistant-enhancements/design-task-mode.md` and the `memory/` folder beside it (read the README index first). Key invariants — breaking these has caused real bugs:

- **Backend never calls an LLM.** All AI work = dispatch a task to an agent via `daemon → agent → runtime`. "PMO plans/summarizes" = enqueue a planning/summary task for the squad leader. See `memory/2026-06-09-llm-decompose-via-leader-task.md`.
- **Lifecycle:** goal_run (migration 112+) flows `discussion → planning → executing → completed/partial/failed`. PMO planning is one-shot (plan + dispatch); a separate **`goal_summary`** task does the final 收口/汇总 once all subtasks are terminal (`memory/2026-06-10-pmo-summary-closeout.md`).
- **FK-less goal tasks** (planning, summary) carry workspace/goal only in `context` JSONB. ANY handler resolving a task's workspace MUST use `TaskService.ResolveTaskWorkspaceID` (it registers `goal_subtask` / `goal_planning` / `goal_summary`). Re-deriving from a subset of FK columns drops `task:message` WS broadcasts → the ④ execution stream silently stays empty. This bug recurred twice.
- **④ execution stream** reuses the chat `TimelineView` (shared in `packages/views/common/task-transcript/`) — do not write a parallel renderer. The API must expose `planning_task_id` / `summary_task_id` / per-subtask `task_id` via `enrichGoalResponse`; the frontend fetches each task's `task_messages`.
