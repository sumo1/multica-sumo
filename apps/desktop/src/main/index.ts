...
// Force Chromium to build the full renderer accessibility (AX) tree eagerly.
// `app.setAccessibilitySupportEnabled(true)` alone only flips the Electron-app
// flag — Chromium still lazily populates the per-renderer AX tree, so external
// consumers (our computer-use harness, which drives the app via the macOS
// Accessibility API) saw an empty AXWebArea with no descendants. This
// command-line switch makes each renderer build its DOM-derived AX tree up
// front. It MUST be set before `app.whenReady()` (Chromium reads switches at
// process init), hence module top-level rather than inside the ready handler.
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    // Force Chromium to always build the accessibility (AX) tree, rather than
    // lazily constructing it only when an assistive technology (VoiceOver)
    // is detected. Without this, external AX consumers — including our
    // computer-use harness, which navigates via the macOS Accessibility API —
    // see only an empty window shell (AXApplication > AXWindow > empty
    // AXGroups) and none of the rendered React UI. Enabling it exposes the
    // full DOM-derived AX tree (buttons, fields, list items) so the harness
    // can locate and operate on real elements.
