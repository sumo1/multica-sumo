...
- `docs/step-{stage-action}/`：二级阶段入口，目录名用英文 kebab-case，并统一加 `step-` 前缀。后续模型遇到“我要验证 / 我要编排 / 我要同步角色 / 我要沉淀”时从这里进。
- `docs/step-{stage-action}/cases/`：具体任务沉淀案例。同一个任务可以在多个阶段留下不同案例。
- `docs/task/{task-id}/`：任务级记录。放需求、方案、计划、任务历史，不负责跨任务导航。
- `docs/task/{task-id}/memory/`：证据层。一条事实一文件，按时间保留原始判断、踩坑、验证结果。

不建宽泛的 `decisions/` 目录，也不建 `actions/` 这种三级杂物间。一个判断如果只在“端到端测试”阶段有用，就放进 `docs/step-e2e-testing/`；如果只在“工程文档沉淀”阶段有用，就放进 `docs/step-repo-docs-persistence/`。目录名必须能回答“我现在要去哪看”。
