...
import {
  ...
  GoalRunSchema,
  GoalRunListSchema,
  GoalPersistResultSchema,
  EMPTY_GOAL_RUN,
  TaskCreateResultSchema,
  EMPTY_TASK_CREATE_RESULT,
  RoleSyncResultSchema,
  EMPTY_ROLE_SYNC_RESULT,
...
export class ApiClient {
  ...
  async createGoal(data: {
    ...
    /** When true, the goal goes straight to executing and dispatches roots. */
    confirmed?: boolean;
    /**
     * When true, dispatch a planning task to the squad leader (PMO) to
     * decompose the goal via LLM. Mutually exclusive with `subtasks`. The
     * returned goal is in `planning`; subtasks appear once the leader submits.
     */
    ...
    subtasks?: Array<{
      ...
      /** Seq values of upstream subtasks this one depends on. */
      depends_on?: number[];
    }>;
  }): Promise<GoalRun> {
  ...
  async getGoal(id: string): Promise<GoalRun> {
  ...
  async confirmGoal(id: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/goals/${id}/confirm`, {
      method: "POST",
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/goals/{id}/confirm",
    });
  }

  /** Persist (one-click snapshot) the task content into its bound project repo.
   *  Returns the dispatched persist task id. The 持久化到工程 entry — the platform
   *  DB stays the main truth; this writes a harness-structured export. */
  async persistGoal(id: string): Promise<{ persist_task_id: string }> {
    const raw = await this.fetch<unknown>(`/api/goals/${id}/persist`, {
      method: "POST",
    });
    return parseWithFallback(raw, GoalPersistResultSchema, { persist_task_id: "" }, {
      endpoint: "POST /api/goals/{id}/persist",
    });
  }

  // Human intervention on a failed / blocked subtask. Each returns the full
  // updated goal (run + subtasks) so the status tree refreshes in one trip.

  // -------------------------------------------------------------------------
  // Task mode (PMO planning layer + dynamic squad)
  // -------------------------------------------------------------------------

  /** List the workspace's tasks (goals), newest first. */
  async listTasks(): Promise<GoalRun[]> {
    const raw = await this.fetch<unknown>("/api/tasks");
    return parseWithFallback(raw, GoalRunListSchema, [], {
      endpoint: "GET /api/tasks",
    });
  }

  /** Create a task in the discussion phase. Returns the goal + discussion chat. */
  async createTask(data: {
    ...
    return parseWithFallback(raw, TaskCreateResultSchema, EMPTY_TASK_CREATE_RESULT, {
      endpoint: "POST /api/tasks",
    });
  }

  /** Sync role definitions from a project's local directory into workspace
   *  Agents (repo → platform, one-way). Returns created/updated/skipped names. */
  async syncProjectRoles(projectId: string): Promise<RoleSyncResult> {
    const raw = await this.fetch<unknown>(`/api/projects/${projectId}/sync-roles`, {
      method: "POST",
    });
    return parseWithFallback(raw, RoleSyncResultSchema, EMPTY_ROLE_SYNC_RESULT, {
      endpoint: "POST /api/projects/{id}/sync-roles",
    });
  }

  async addTaskMember(taskId: string, agentId: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/tasks/${taskId}/members`, {
      method: "POST",
      body: JSON.stringify({ agent_id: agentId }),
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/tasks/{id}/members",
    });
  }

  async confirmTask(taskId: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/tasks/${taskId}/confirm`, {
      method: "POST",
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
  ...
  async setWorkspacePlanner(agentId: string): Promise<void> {
    await this.fetch(`/api/tasks/planner`, {
      method: "PUT",
      body: JSON.stringify({ agent_id: agentId }),
    });
  }

  async retryGoalSubtask(subtaskId: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/goals/subtasks/${subtaskId}/retry`, {
      method: "POST",
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/goals/subtasks/{id}/retry",
    });
  }

  async reassignGoalSubtask(subtaskId: string, agentId: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/goals/subtasks/${subtaskId}/reassign`, {
      method: "POST",
      body: JSON.stringify({ agent_id: agentId }),
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/goals/subtasks/{id}/reassign",
    });
  }

  async editGoalSubtaskSpec(subtaskId: string, spec: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/goals/subtasks/${subtaskId}/edit-spec`, {
      method: "POST",
      body: JSON.stringify({ spec }),
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/goals/subtasks/{id}/edit-spec",
    });
  }

  async skipGoalSubtask(subtaskId: string): Promise<GoalRun> {
    const raw = await this.fetch<unknown>(`/api/goals/subtasks/${subtaskId}/skip`, {
      method: "POST",
    });
    return parseWithFallback(raw, GoalRunSchema, EMPTY_GOAL_RUN, {
      endpoint: "POST /api/goals/subtasks/{id}/skip",
    });
  }

  /**
   * Open a human-takeover chat for a failed subtask. Returns the new chat
   * session (bound to the subtask's agent, carrying the subtask context) so the
   * UI can switch the chat surface to it.
   */
  async takeoverGoalSubtask(subtaskId: string): Promise<ChatSession> {
    const raw = await this.fetch<unknown>(`/api/goals/subtasks/${subtaskId}/takeover`, {
      method: "POST",
    });
    return parseWithFallback(raw, ChatSessionSchema, EMPTY_CHAT_SESSION, {
      endpoint: "POST /api/goals/subtasks/{id}/takeover",
    });
  }
