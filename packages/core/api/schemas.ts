...
// ---------------------------------------------------------------------------
// Chat sessions. Drives the Assistant page; runtime_id was added when chat
// sessions gained per-conversation runtime binding. Defaults keep older
// servers (no runtime_id field) and list-vs-single shape differences
// (has_unread is list-only) from white-screening the page.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Goal mode (PMO orchestration). Status enums use .catch() so a future
// server-side status value downgrades to a safe default instead of white-
// screening the status tree (enum-drift rule). depends_on defaults to [] so a
// null array from an older server doesn't throw.
// ---------------------------------------------------------------------------

export const GoalSubtaskSchema = z.object({
  id: z.string(),
  goal_run_id: z.string(),
  ...
  kind: z.enum(["execute", "verify"]).catch("execute"),
  verdict: z.enum(["", "pass", "reject"]).catch(""),
  attempt: z.number().default(0),
  max_attempts: z.number().default(2),
  failure_reason: z.string().default(""),
  result: z.unknown().optional(),
  upstream_output: z.string().default(""),
  handoff_brief: z.string().default(""),
...
export const GoalRunSchema = z.object({
  ...
  subtasks: z.array(GoalSubtaskSchema).default([]),
  planning_task_id: z.string().default(""),
  summary_task_id: z.string().default(""),
  confirmed_at: z.string().default(""),
  project_id: z.string().default(""),
  persist_task_id: z.string().default(""),
  can_persist: z.boolean().default(false),
  coordinator_name: z.string().default(""),
  coordinator_runtime_name: z.string().default(""),
  coordinator_runtime_provider: z.string().default(""),
  coordinator_model: z.string().default(""),
  created_at: z.string().default(""),
  updated_at: z.string().default(""),
}).loose();

export const EMPTY_GOAL_RUN: GoalRun = {
  id: "",
  workspace_id: "",
  squad_id: "",
  chat_session_id: "",
...
export const GoalRunListSchema = z.array(GoalRunSchema);

/** POST /api/goals/{id}/persist response: the dispatched persist task id. */
export const GoalPersistResultSchema = z.object({
  persist_task_id: z.string().default(""),
}).loose();

// Task-mode create response: goal + the discussion chat id to open.
export const TaskCreateResultSchema = z.object({
  goal: GoalRunSchema,
  discussion_chat_id: z.string().default(""),
}).loose();

export const EMPTY_TASK_CREATE_RESULT: TaskCreateResult = {
  goal: EMPTY_GOAL_RUN,
  discussion_chat_id: "",
};

// Role sync result: created/updated/skipped agent names.
export const RoleSyncResultSchema = z.object({
  created: z.array(z.string()).default([]),
  updated: z.array(z.string()).default([]),
  skipped: z.array(z.string()).default([]),
}).loose();

export const EMPTY_ROLE_SYNC_RESULT: RoleSyncResult = {
  created: [],
  updated: [],
  skipped: [],
};
