...
export function useRealtimeSync(
  ...
  useEffect(() => {
    ...
    const refreshMap: Record<string, () => void> = {
      ...
      goal: () => {
        // goal:run_updated / goal:subtask_updated — refetch the goal_run(s) so
        // the status tree reflects lifecycle + per-subtask transitions. Keyed
        // invalidation under ["goals", wsId] covers every open goal_run.
        const wsId = getCurrentWsId();
