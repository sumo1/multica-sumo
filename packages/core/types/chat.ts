export interface ChatSession {
  ...
  /**
   * Runtime this session is bound to, chosen at create time and immutable.
   * Empty string when the session inherited the agent's default runtime
   * (the server returns "" for a NULL column). The UI uses it to show which
   * runtime the conversation runs on.
   */
  runtime_id: string;
