...
export function RuntimePicker({
  ...
  /**
   * When true, offline runtimes are non-selectable. Used by chat session
   * creation, where the runtime binding is immutable — binding an offline
   * runtime would create a conversation that can never run. Agent creation
   * leaves this false: an agent may legitimately bind a runtime that's
   * currently offline but will come online later.
   */
  ...
  // Combined selectability: visibility gate + (optionally) online status.
  const isSelectable = (r: RuntimeDevice) =>
    isRuntimeUsableForUser(r, currentUserId) &&
    (!blockOffline || r.status === "online");
  ...
  useEffect(() => {
    ...
    const firstUsable = filteredRuntimes.find(isSelectable);
    if (firstUsable) onSelect(firstUsable.id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filteredRuntimes, selectedRuntimeId, currentUserId, onSelect, blockOffline]);
  ...
  const handleFilterChange = (next: RuntimeFilter) => {
    ...
    const firstUsable = nextList.find(isSelectable);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {filteredRuntimes.map((device) => {
            ...
            const lockedByVisibility = !isRuntimeUsableForUser(
              device,
              currentUserId,
            );
            const lockedByOffline =
              !lockedByVisibility && blockOffline && device.status !== "online";
            const disabled = lockedByVisibility || lockedByOffline;
            const disabledTitle = lockedByVisibility
              ? t(($) => $.create_dialog.runtime_private_locked_tooltip)
              : lockedByOffline
                ? t(($) => $.create_dialog.runtime_offline_tooltip)
                : undefined;
            return (
              <button
                key={device.id}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {lockedByOffline && (
                      <span className="shrink-0 inline-flex items-center gap-1 rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                        {t(($) => $.create_dialog.runtime_offline_badge)}
                      </span>
                    )}
                  </div>
