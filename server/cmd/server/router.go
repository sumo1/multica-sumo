		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/projects", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					// Sync role definitions from the project's local directory
					// into workspace Agents (repo → platform, one-way).
					r.Post("/sync-roles", h.SyncProjectRoles)
				})
			})
			...
			// Tasks (task mode: PMO planning layer + dynamic squad).
			...
			// Goals (PMO orchestration over a squad)
			r.Route("/api/goals", func(r chi.Router) {
				r.Post("/", h.CreateGoal)
				r.Route("/{id}", func(r chi.Router) {
					r.Get("/", h.GetGoal)
					r.Post("/confirm", h.ConfirmGoal)
					r.Post("/plan", h.SubmitPlan)
					r.Post("/persist", h.PersistGoal)
				})
				// Subtask-scoped: verdict (verify nodes) + human interventions.
				r.Route("/subtasks/{subtaskId}", func(r chi.Router) {
					...
					r.Post("/skip", h.SkipSubtask)
					r.Post("/takeover", h.TakeoverSubtask)
				})
			})
