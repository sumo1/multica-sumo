...
type Config struct {
	...
	AgentEnv                       map[string]string     // provider credential/relay/proxy env vars recovered from the user's login shell when the daemon process itself lacks them (GUI launch). Injected as the base env for every agent subprocess; per-agent CustomEnv still overrides. See resolveAgentEnvViaLoginShell.
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	return Config{
		...
		Profile:                        profile,
		Agents:                         agents,
		AgentEnv:                       resolveAgentEnvViaLoginShell(),
...
// agentEnvAllowPrefixes is the whitelist of environment-variable name prefixes
// the daemon will carry over from the user's login shell into agent
// subprocesses. These are the variables agent CLIs read to reach their model
// backend — API keys, custom/relay base URLs, Bedrock/Vertex routing, and
// outbound proxy settings. A GUI-launched daemon (Electron) inherits none of
// the user's interactive shell env, so without this an agent that depends on a
// relay endpoint or proxy (rather than a standard on-disk auth.json) fails with
// "Invalid API key" / "Missing environment variable". Prefix matching keeps the
// list maintainable as providers add variables; the surface is deliberately
// limited to credential/endpoint/proxy namespaces and never general shell env.
var agentEnvAllowPrefixes = []string{
	"ANTHROPIC_",
	"CLAUDE_CODE_",
	...
	"CODEX_",
	"GEMINI_",
	"GOOGLE_", // GOOGLE_API_KEY / GOOGLE_VERTEX_BASE_URL / GOOGLE_GENAI_USE_VERTEXAI
	"VERTEX_",
	"COPILOT_",
	"CURSOR_",
}

// agentEnvAllowExact covers proxy variables, whose names don't share a clean
// prefix. Both cases are matched (curl/Go honor the lowercase forms).
var agentEnvAllowExact = map[string]struct{}{
	"HTTP_PROXY": {}, "HTTPS_PROXY": {}, "ALL_PROXY": {}, "NO_PROXY": {},
	"http_proxy": {}, "https_proxy": {}, "all_proxy": {}, "no_proxy": {},
}

// isAllowedAgentEnvKey reports whether an env var name is in the agent-env
// whitelist (credential / endpoint / proxy namespaces only).
func isAllowedAgentEnvKey(key string) bool {
	if _, ok := agentEnvAllowExact[key]; ok {
		return true
	}
	upper := strings.ToUpper(key)
	for _, p := range agentEnvAllowPrefixes {
...
// resolveAgentEnvViaLoginShell recovers the whitelisted provider env (see
// agentEnvAllowPrefixes) from the user's interactive login shell, so a daemon
// launched without a shell (Electron GUI, Launchpad) can still run agent CLIs
// that depend on relay endpoints / proxies configured in ~/.zshrc et al.
//
// It mirrors resolveAgentsViaLoginShell: same supported-shell gate, same
// `$SHELL -ilc` invocation, same timeout/wait-delay budget. A variable already
// present in the daemon's own environment is NOT overridden — an explicitly set
// value (standalone daemon started from a terminal, or an operator-provided
// env) always wins over what the login shell reports. Returns an empty map on
// any failure; callers treat that as "nothing to inject".
func resolveAgentEnvViaLoginShell() map[string]string {
	out := map[string]string{}
	...
	// `env` in a login shell prints NAME=VALUE per line. We filter to the
	// whitelist on the Go side rather than in the script so the matching logic
	// lives in one place (isAllowedAgentEnvKey).
	...
	for _, line := range strings.Split(string(raw), "\n") {
		...
		// Don't override a value the daemon already has — explicit wins.
