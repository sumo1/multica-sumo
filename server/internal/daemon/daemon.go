...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// Provider credential/relay/proxy env recovered from the user's login
	// shell (see resolveAgentEnvViaLoginShell). Applied as the base layer so a
	// GUI-launched daemon can reach relay endpoints the same way a
	// terminal-launched one does. Daemon-set MULTICA_* keys above are never in
	// the whitelist, and per-agent CustomEnv below still overrides these.
	for k, v := range d.cfg.AgentEnv {
		agentEnv[k] = v
	}
