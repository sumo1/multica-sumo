...
type CreateChatSessionRequest struct {
	...
	// RuntimeID, when set, binds the session to a specific runtime instead of
	// inheriting the agent's default. Optional: nil/empty falls back to the
	// agent's runtime_id (preserves legacy behavior). Bound at create time and
	// immutable thereafter — switching runtime means a new session.
...
func (h *Handler) CreateChatSession(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve the runtime binding. When the caller passes a runtime_id we
	// validate its format and confirm it lives in this workspace, then bind
	// the session to it. When absent, RuntimeID stays invalid and the SQL
	// falls back to the agent's default runtime (legacy behavior).
	// Online-state is enforced client-side only (RuntimePicker disables
	// offline runtimes); see design 1.4 for the trade-off.
	var runtimeID pgtype.UUID
	if req.RuntimeID != nil && strings.TrimSpace(*req.RuntimeID) != "" {
		parsed, ok := parseUUIDOrBadRequest(w, strings.TrimSpace(*req.RuntimeID), "runtime_id")
		if !ok {
			return
		}
		if _, err := h.Queries.GetAgentRuntimeForWorkspace(r.Context(), db.GetAgentRuntimeForWorkspaceParams{
			...
			writeError(w, http.StatusBadRequest, "runtime not found in workspace")
			return
		}
		runtimeID = parsed
	}
...
type ChatSessionResponse struct {
	...
	Title       string `json:"title"`
	Status      string `json:"status"`
	// RuntimeID is the runtime this session is bound to (chosen at create time,
	// immutable). Empty string when the session inherited the agent default and
	// the column is NULL. Lets the UI show "this conversation runs on <runtime>".
	RuntimeID string `json:"runtime_id"`
