...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// Takeover chat: when this session is bound to a goal subtask, surface
			// the subtask's title / spec / failure so the daemon's chat prompt can
			// orient the agent ("you were working on X, it failed because Y").
			if cs.GoalSubtaskID.Valid {
				if st, err := h.Queries.GetGoalSubtask(r.Context(), cs.GoalSubtaskID); err == nil {
					resp.TakeoverSubtaskTitle = st.Title
					resp.TakeoverSubtaskSpec = st.Spec
					if st.FailureReason.Valid {
						resp.TakeoverFailureReason = st.FailureReason.String
					}
				}
			}
	...
	// Goal-mode subtask task: like quick-create, it has context JSONB and no
	// issue/chat/autopilot link. The spec is the prompt; workspace and the
	// goal/subtask titles come from the context so the daemon's
	// buildGoalSubtaskPrompt has everything without a DB round-trip.
	if task.Context != nil && task.GoalSubtaskID.Valid &&
		!task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		var gc service.GoalSubtaskContext
		if json.Unmarshal(task.Context, &gc) == nil && gc.Type == service.GoalSubtaskContextType {
			resp.GoalSubtaskID = gc.GoalSubtaskID
			resp.GoalTitle = gc.GoalTitle
			resp.GoalSubtaskTitle = gc.SubtaskTitle
			resp.GoalSubtaskSpec = gc.Spec
			resp.GoalSubtaskKind = gc.Kind
			...
			if len(resp.Repos) == 0 {
				if wsUUID, err := util.ParseUUID(gc.WorkspaceID); err == nil {
	...
	// Goal-planning task: the squad leader (PMO) decomposes the goal. Carries
	// context JSONB (no FK), like quick-create. We surface the goal text +
	// run id for the prompt, and inject the squad roster into the leader's
	// instructions so it knows which role agents (and UUIDs) to assign.
	if task.Context != nil &&
		!task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid && !task.GoalSubtaskID.Valid {
		var gp service.GoalPlanningContext
		if json.Unmarshal(task.Context, &gp) == nil && gp.Type == service.GoalPlanningContextType {
			...
			if len(resp.Repos) == 0 {
				if wsUUID, err := util.ParseUUID(gp.WorkspaceID); err == nil {
					if ws, err := h.Queries.GetWorkspace(r.Context(), wsUUID); err == nil && ws.Repos != nil {
						var repos []RepoData
						if json.Unmarshal(ws.Repos, &repos) == nil && len(repos) > 0 {
							resp.Repos = repos
			...
			// Inject the planning roster so the planner can assign role agents.
			// Unlike the issue-squad briefing, this widens the roster to ALL
			// usable workspace agents (not just squad members): a task's dynamic
			// squad often starts with only the leader, and the planner must be
			// able to assign nodes to the workspace's role agents (coder /
			// reviewer / evaluator …). dispatchSubtask only requires a non-
			// archived workspace agent with a runtime — NOT squad membership — so
			// any pool agent is dispatchable. When the goal is bound to a project,
			// that repo's harness roles are flagged as the project's own.
			if resp.Agent != nil {
				if squadUUID, err := util.ParseUUID(gp.SquadID); err == nil {
					if squad, err := h.Queries.GetSquad(r.Context(), squadUUID); err == nil {
						roster := h.buildPlanningRoster(r.Context(), squad, gp.ProjectID)
						if strings.TrimSpace(resp.Agent.Instructions) == "" {
							resp.Agent.Instructions = roster
						} else {
							resp.Agent.Instructions = resp.Agent.Instructions + "\n\n" + roster
						}
					}
			...
			// Run the planner inside the bound project's repo so it can read the
			// project's existing docs/task contracts and align to its dialect.
			h.attachProjectContext(r.Context(), &resp, gp.ProjectID)
		}
	}

	// Goal-summary task: the squad leader (PMO) writes the final deliverable by
	// synthesizing the subtask outputs. FK-less + context JSONB, like planning,
	// but its reply IS the result (no CLI write-back, no roster injection).
	if task.Context != nil &&
		!task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid && !task.GoalSubtaskID.Valid {
		var gs service.GoalSummaryContext
		if json.Unmarshal(task.Context, &gs) == nil && gs.Type == service.GoalSummaryContextType {
			resp.GoalSummaryRunID = gs.GoalRunID
			resp.GoalSummaryGoal = gs.Goal
			resp.GoalSummaryOutcome = gs.Outcome
			resp.GoalSummaryDigest = gs.SubtaskDigest
			resp.GoalTitle = gs.GoalTitle
			if resp.WorkspaceID == "" {
				resp.WorkspaceID = gs.WorkspaceID
			}
			if len(resp.Repos) == 0 {
				if wsUUID, err := util.ParseUUID(gs.WorkspaceID); err == nil {
					if ws, err := h.Queries.GetWorkspace(r.Context(), wsUUID); err == nil && ws.Repos != nil {
						var repos []RepoData
						if json.Unmarshal(ws.Repos, &repos) == nil && len(repos) > 0 {
							resp.Repos = repos
	...
	// Goal-persist task: the 总控 snapshots the task content into the project
	// repo. FK-less + context JSONB like planning/summary, BUT the agent must run
	// INSIDE the project's local_directory to author files there — so unlike the
	// other goal tasks we surface ProjectID + ProjectResources, which is how the
	// daemon resolves the work dir (findLocalDirectoryAssignment).
	if task.Context != nil &&
		!task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid && !task.GoalSubtaskID.Valid {
		var gp service.GoalPersistContext
		if json.Unmarshal(task.Context, &gp) == nil && gp.Type == service.GoalPersistContextType {
			resp.GoalPersistRunID = gp.GoalRunID
			resp.GoalPersistGoal = gp.Goal
			resp.GoalPersistSlug = gp.Slug
			resp.GoalPersistOutcome = gp.Outcome
			resp.GoalPersistDigest = gp.SubtaskDigest
			resp.GoalTitle = gp.GoalTitle
			if resp.WorkspaceID == "" {
				resp.WorkspaceID = gp.WorkspaceID
			}
			// Surface the bound project's resources so the daemon runs the agent
			// inside its local_directory (where the harness files get written).
			h.attachProjectContext(r.Context(), &resp, gp.ProjectID)
		}
	}

	// Goal-decision task: the 总控 judges a failed node's next step (下一步判断).
	// FK-less + context JSONB like planning/summary; the reply is reported via
	// `multica goal decide`. No roster, no repo — pure judgment.
	if task.Context != nil &&
		!task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid && !task.GoalSubtaskID.Valid {
		var gd service.GoalDecisionContext
		if json.Unmarshal(task.Context, &gd) == nil && gd.Type == service.GoalDecisionContextType {
			resp.GoalDecisionSubtaskID = gd.GoalSubtaskID
			resp.GoalDecisionSubtaskTitle = gd.SubtaskTitle
			...
			if len(resp.Repos) == 0 {
				if wsUUID, err := util.ParseUUID(gd.WorkspaceID); err == nil {
					if ws, err := h.Queries.GetWorkspace(r.Context(), wsUUID); err == nil && ws.Repos != nil {
						var repos []RepoData
						if json.Unmarshal(ws.Repos, &repos) == nil && len(repos) > 0 {
							resp.Repos = repos
...
func (h *Handler) ReportTaskMessages(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve via the shared resolver so goal-mode tasks (goal_subtask /
	// goal_planning) and quick-create tasks broadcast their live stream too —
	// not just issue / chat tasks. Without this the messages are persisted but
	// the task:message WS event is dropped, so the Task page ④ column never
	// fills until a manual refetch (it relies on the WS push because
	// taskMessagesOptions uses staleTime: Infinity).
