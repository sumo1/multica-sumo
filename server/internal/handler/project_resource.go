...
// attachProjectContext sets resp.ProjectID/ProjectTitle and maps the project's
// resources onto resp.ProjectResources, given a project id string from a task's
// context JSONB. This is what lets a goal task's agent run INSIDE the bound
// project's local_directory (the daemon resolves the work dir from
// ProjectResources via findLocalDirectoryAssignment) — so the agent can read the
// project's existing docs/task contracts and align to that project's dialect.
// No-op on an empty/invalid id. Shared by the planning / subtask / persist claim
// paths so they surface project context identically.
func (h *Handler) attachProjectContext(ctx context.Context, resp *AgentTaskResponse, projectID string) {
	if projectID == "" {
		return
	}
	projectUUID, err := parseUUIDLoose(projectID)
	if err != nil {
		return
	}
	resp.ProjectID = projectID
	if proj, err := h.Queries.GetProject(ctx, projectUUID); err == nil {
