...
// TestBuildPlanningRoster_WidensToWorkspacePool locks the fix for "all subtasks
// went to the same agent": the PLANNING roster must include usable workspace
// agents that are NOT squad members, as an assignable pool — so a task whose
// dynamic squad holds only the leader can still be decomposed across roles.
func TestBuildPlanningRoster_WidensToWorkspacePool(t *testing.T) {
	ctx := context.Background()
	leaderID, leaderName := seededLeaderAgent(t)
	// Squad with ONLY the leader (the exact situation in the bug screenshot).
	squad := seedSquadForBriefing(t, leaderID, "Solo Planner Squad", "")

	// A separate workspace agent, NOT added to the squad.
	poolAgent := createHandlerTestAgent(t, "Pool Coder", []byte("[]"))

	out := testHandler.buildPlanningRoster(ctx, squad, "")

	// Leader still shown; the non-member workspace agent appears in the pool.
	if !strings.Contains(out, leaderName) {
		t.Errorf("planning roster must still show the leader")
	}
	if !strings.Contains(out, "Available workspace roles") {
		t.Errorf("planning roster must include the workspace pool section, got:\n%s", out)
	}
	if !strings.Contains(out, "`[@Pool Coder](mention://agent/"+poolAgent+")`") {
		t.Errorf("planning roster must list the non-squad workspace agent as assignable, got:\n%s", out)
	}
	if !strings.Contains(out, "need NOT be squad members") {
		t.Errorf("planning roster should tell the planner pool agents are directly assignable, got:\n%s", out)
	}
}

// TestBuildPlanningRoster_IncludesRoleDescriptions locks the fix for "the role
// choices look strange": the planner picked roles by NAME alone because the
// roster omitted each role's description. The roster must carry each role's
// description (its specialty), truncated, so the planner can match a node to
// what the role actually does — not guess from the name.
func TestBuildPlanningRoster_IncludesRoleDescriptions(t *testing.T) {
	ctx := context.Background()
	leaderID, _ := seededLeaderAgent(t)
	squad := seedSquadForBriefing(t, leaderID, "Desc Squad", "")

	// Create a workspace agent WITH a description (the specialty text).
	const desc = "Adversarial code reviewer: stands at an independent perspective before every commit and hunts for missed edge cases."
	var agentID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO agent (workspace_id, name, description, runtime_mode, runtime_config,
			runtime_id, visibility, max_concurrent_tasks, owner_id)
		VALUES ($1, 'code-reviewer', $2, 'cloud', '{}'::jsonb, $3, 'private', 6, $4)
		RETURNING id
	`, testWorkspaceID, desc, handlerTestRuntimeID(t), testUserID).Scan(&agentID); err != nil {
		t.Fatalf("create described agent: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM agent WHERE id = $1`, agentID) })

	out := testHandler.buildPlanningRoster(ctx, squad, "")

	// The description (specialty) must appear in the roster so the planner can
	// match nodes by role, not by name.
	if !strings.Contains(out, "Adversarial code reviewer") {
		t.Errorf("planning roster must include the role's description so the planner picks by specialty, got:\n%s", out)
	}
}
...
func TestTruncateRoleDesc(t *testing.T) {
	if got := truncateRoleDesc("  "); got != "" {
		t.Errorf("blank desc → empty, got %q", got)
	}
	// First paragraph only + whitespace collapsed.
	if got := truncateRoleDesc("First line.\n\nSecond paragraph."); got != "First line." {
		t.Errorf("first-paragraph only, got %q", got)
	}
	// Long desc capped with an ellipsis.
	long := strings.Repeat("x", 300)
	got := truncateRoleDesc(long)
	if r := []rune(got); len(r) > 201 || !strings.HasSuffix(got, "…") {
		t.Errorf("long desc must be capped + …, got len=%d suffix-ok=%v", len([]rune(got)), strings.HasSuffix(got, "…"))
	}
}
