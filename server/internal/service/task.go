...
func (s *TaskService) EnqueueChatTask(ctx context.Context, chatSession db.ChatSession) (db.AgentTaskQueue, error) {
	...
	// Runtime resolution: the session's bound runtime wins (chosen at create
	// time), falling back to the agent's default when the session never bound
	// one (legacy sessions, or create without an explicit choice). This is what
	// makes the user's per-conversation runtime selection actually take effect —
	// previously the task always ran on the agent default.
	runtimeID := chatSession.RuntimeID
	if !runtimeID.Valid {
		runtimeID = agent.RuntimeID
	}
	if !runtimeID.Valid {
		return db.AgentTaskQueue{}, fmt.Errorf("no runtime for chat session: neither session nor agent has one")
	}
...
func (s *TaskService) ClaimTask(ctx context.Context, agentID pgtype.UUID) (*db.AgentTaskQueue, error) {
	...
	// Validate the agent still exists (a claim for a deleted agent must fail),
	// but DO NOT apply a per-agent concurrency admission gate. We deliberately
	// do NOT hold tasks in 'queued' waiting for a free max_concurrent_tasks
	// slot: dispatch immediately to the assigned agent. If the runtime genuinely
	// can't handle the load, ITS real error (rate limit / overloaded / provider
	// 429) surfaces through FailTask(result.Comment) and is shown to the user —
	// we don't proactively queue or fabricate a "no capacity" outcome. Per-issue
	// / per-chat serialization (a correctness invariant, not a capacity limit)
	// still lives in ClaimAgentTask's SQL, and the daemon's own slot semaphore
	// is the machine-level backstop against fork-bombing the host.
	t0 := start
...
func (s *TaskService) ResolveTaskWorkspaceID(ctx context.Context, task db.AgentTaskQueue) string {
	...
	// Goal-mode subtask tasks resolve workspace via goal_subtask → goal_run.
	// Same rationale as quick-create above: without this the task:* broadcasts
	// are dropped and the goal listener never advances the DAG.
	if task.GoalSubtaskID.Valid {
		if st, err := s.Queries.GetGoalSubtask(ctx, task.GoalSubtaskID); err == nil {
			if run, err := s.Queries.GetGoalRun(ctx, st.GoalRunID); err == nil {
				return util.UUIDToString(run.WorkspaceID)
			}
		}
	}
	// Goal-planning AND goal-summary tasks carry no FK; workspace lives in the
	// context JSONB (same as quick-create). Without this their task:message
	// broadcasts drop and the ④ main-session stream never fills live.
	if ws := goalContextWorkspaceID(task); ws != "" {
		return ws
	}
	return ""
}

// goalContextWorkspaceID extracts the workspace id from a FK-less goal task's
// context JSONB (planning or summary), or "" when the task is neither. Both
// shapes carry `type` + `workspace_id` at the top level, so one minimal decode
// covers both.
func goalContextWorkspaceID(task db.AgentTaskQueue) string {
	if task.IssueID.Valid || task.ChatSessionID.Valid || task.AutopilotRunID.Valid || task.GoalSubtaskID.Valid {
		return ""
	}
	...
	switch gc.Type {
	case GoalPlanningContextType, GoalSummaryContextType, GoalPersistContextType, GoalDecisionContextType:
		return gc.WorkspaceID
	default:
		return ""
	}
}
