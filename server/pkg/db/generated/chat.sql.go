...
VALUES (
    $1, $2, $3, $4,
    COALESCE($5, (SELECT runtime_id FROM agent WHERE id = $2))
)
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, goal_subtask_id, goal_run_id
`
...
type CreateChatSessionParams struct {
	...
	RuntimeID   interface{} `json:"runtime_id"`
}
...
const createDiscussionChatSession = `-- name: CreateDiscussionChatSession :one
INSERT INTO chat_session (workspace_id, agent_id, creator_id, title, runtime_id, goal_run_id)
VALUES (
    $1, $2, $3, $4,
    COALESCE($6, (SELECT runtime_id FROM agent WHERE id = $2)),
    $5
)
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, goal_subtask_id, goal_run_id
`

type CreateDiscussionChatSessionParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	AgentID     pgtype.UUID `json:"agent_id"`
	CreatorID   pgtype.UUID `json:"creator_id"`
	Title       string      `json:"title"`
...
func (q *Queries) CreateDiscussionChatSession(ctx context.Context, arg CreateDiscussionChatSessionParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, createDiscussionChatSession,
		arg.WorkspaceID,
		arg.AgentID,
		arg.CreatorID,
		arg.Title,
...
const createTakeoverChatSession = `-- name: CreateTakeoverChatSession :one
...
type CreateTakeoverChatSessionParams struct {
	WorkspaceID   pgtype.UUID `json:"workspace_id"`
	AgentID       pgtype.UUID `json:"agent_id"`
	CreatorID     pgtype.UUID `json:"creator_id"`
	Title         string      `json:"title"`
...
func (q *Queries) CreateTakeoverChatSession(ctx context.Context, arg CreateTakeoverChatSessionParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, createTakeoverChatSession,
		arg.WorkspaceID,
		arg.AgentID,
		arg.CreatorID,
		arg.Title,
...
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, goal_subtask_id, goal_run_id FROM chat_session
WHERE id = $1
`
...
const getDiscussionChatForGoal = `-- name: GetDiscussionChatForGoal :one
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, goal_subtask_id, goal_run_id FROM chat_session
WHERE goal_run_id = $1
...
func (q *Queries) GetDiscussionChatForGoal(ctx context.Context, goalRunID pgtype.UUID) (ChatSession, error) {
	row := q.db.QueryRow(ctx, getDiscussionChatForGoal, goalRunID)
	var i ChatSession
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT cs.id, cs.workspace_id, cs.agent_id, cs.creator_id, cs.title, cs.session_id, cs.work_dir, cs.status, cs.created_at, cs.updated_at, cs.unread_since, cs.runtime_id, cs.goal_subtask_id, cs.goal_run_id,
