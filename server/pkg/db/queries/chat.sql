...
-- runtime_id binds the session to a specific runtime. When the caller passes
-- a runtime (sqlc.narg), use it; otherwise fall back to the agent's default
-- runtime so legacy callers and sessions without an explicit choice behave
-- exactly as before. The binding is immutable after create.
INSERT INTO chat_session (workspace_id, agent_id, creator_id, title, runtime_id)
VALUES (
    $1, $2, $3, $4,
    COALESCE(sqlc.narg('runtime_id'), (SELECT runtime_id FROM agent WHERE id = $2))
)
RETURNING *;

-- name: CreateTakeoverChatSession :one
-- Create a chat session that is the human-takeover conversation for a goal
-- subtask. Binds the subtask's assignee agent + runtime and stamps
-- goal_subtask_id so the daemon's chat prompt can inject the subtask context.
INSERT INTO chat_session (workspace_id, agent_id, creator_id, title, runtime_id, goal_subtask_id)
VALUES (
    $1, $2, $3, $4,
    COALESCE(sqlc.narg('runtime_id'), (SELECT runtime_id FROM agent WHERE id = $2)),
    $5
...
-- name: CreateDiscussionChatSession :one
-- Create the discussion chat session for a goal_run (task mode): the multi-round
-- conversation with the PMO/planner agent that forms the requirement. Bound to
-- the PMO agent and stamped with goal_run_id.
...
-- name: GetDiscussionChatForGoal :one
-- The discussion chat session for a goal, if one exists.
