...
const (
	...
	// Goal events (PMO orchestration). The frontend subscribes by `goal:`
	// prefix and invalidates the goal_run + subtasks snapshot, so the status
	// tree reflects lifecycle and per-subtask transitions in realtime.
	EventGoalRunUpdated     = "goal:run_updated"     // goal_run status / fields changed
	EventGoalSubtaskUpdated = "goal:subtask_updated" // a goal_subtask status changed
